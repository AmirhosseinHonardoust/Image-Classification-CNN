import argparse, os, json, torch, torch.nn as nn, torch.optim as optim
from torch.utils.data import DataLoader
from torchvision import datasets, transforms
from tqdm import tqdm
from utils import device_select, plot_curves, save_sample_predictions
...
def train_loop(model, train_loader, val_loader, device, epochs, outdir, lr=1e-3, patience=3):
    ...
    for ep in range(1, epochs+1):
        ...
        first_batch_imgs, first_batch_lbls, first_batch_preds = None, None, None
        with torch.no_grad():
        ...
        if val_acc > best_val_acc:
            ...
            stale = 0
            save_sample_predictions(first_batch_imgs.cpu(), first_batch_lbls.cpu(), first_batch_preds.cpu(),
                                    classes=[str(i) for i in range(10)], 
                                    outpath=os.path.join(outdir, "sample_predictions.png"))
        else:
            stale += 1
        print(f"[epoch {ep}] train_loss={train_loss:.4f} val_loss={val_loss:.4f} train_acc={train_acc:.4f} val_acc={val_acc:.4f}")
        if stale >= patience:
...
def main():
    ...
    best_path, best_val_acc, history = train_loop(model, train_loader, val_loader, device, args.epochs, args.outdir, lr=args.lr)
